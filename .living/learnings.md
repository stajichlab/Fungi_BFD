...
**What happened**: In `Fungi_BFD_runs/genome_annotation/<SPECIES>/training`, the `training` folder is a symlink meant to point at the project-local `Fungi_BFD_runs/genome_annotation_training/<SPECIES>/training` directory. For a substantial number of species this symlink was broken (dangling — pointing at a path that no longer existed or was never the right target), rather than resolving to the real per-species training data. This silently blocked the `predict` step from finding training data for those genomes; the failure mode was cryptic (no clear "missing training data" error pointing at the symlink itself). Fixed by removing the broken links: `find . -maxdepth 1 -xtype l -exec rm {} \;` run inside `genome_annotation` to strip out just the dangling symlinks (`-xtype l` matches links whose target does not exist), leaving valid symlinks and non-symlink files untouched.

**Why it matters**: A dangling symlink for `training` isn't a "missing training data" error — funannotate/predict just proceeds without training data (or fails downstream in a way that doesn't point back to the symlink), so genomes silently ran without training data or failed for reasons that looked unrelated to the actual root cause. This is easy to miss at BFD's genome count (thousands of per-species directories) because nothing surfaces the broken link short of an explicit `-xtype l` scan.

**Resolution**: Removed broken `training` symlinks under `genome_annotation/*/training` via `find . -maxdepth 1 -xtype l -exec rm {} \;`. Not yet resolved: (1) whether removal alone is sufficient, or whether the correct symlink needs to be recreated per-species pointing at `genome_annotation_training/<SPECIES>/training`; (2) no pre-flight check currently exists before the predict step to catch this class of corruption before it silently degrades a run. Suggested follow-up checks:
- Before starting `predict`, for each species with a `genome_annotation/<SPECIES>/training` entry: confirm it is a symlink (`-xtype l` / `test -L`) AND that it resolves (`test -e`, not just `-L`) to a real directory with expected training files inside.
- If `training` exists but is a broken symlink, a symlink pointing at the wrong target, a regular directory instead of a symlink, or missing entirely when `genome_annotation_training/<SPECIES>/training` exists — flag it explicitly as a config/corruption problem rather than silently proceeding without training data.
- Consider a periodic repo-wide `-xtype l` sweep across `genome_annotation/*/training` (and any other place that symlinks into a project-local sibling directory) as a health check, since this class of breakage is invisible until predict quietly underperforms.

**Tags**: symlink, genome_annotation, genome_annotation_training, funannotate, predict, training-data, broken-link, xtype-l, data-integrity, preflight-check

**mitigation_type**: ambient-awareness

**structural_mitigation_candidate**: A pre-`predict` validation step (script or Nextflow process) that, for every species directory, checks `training` is a symlink AND resolves to an existing directory matching `genome_annotation_training/<SPECIES>/training`, and hard-fails or flags the species instead of silently continuing. Not yet implemented — currently only fixed reactively via manual `find -xtype l` cleanup. **Update 2026-09-10**: `scripts/fix_training_symlinks.py` written and run in `Fungi_BFD_runs` (`--apply`). Root cause confirmed precisely: `genome_annotation_training/<SPECIES>/training` is the single canonical training-output directory per species; `do_annotation*/genome_annotation_training` are themselves relative symlinks (`../genome_annotation_training`) back to that one canonical dir (confirmed via matching inodes). Broken `genome_annotation/<SPECIES>/training` symlinks (independently found and removed by the user via a separate `find -xtype l -exec rm {} \;` pass) had pointed at `.../BFD/Fungi_BFD/do_annotation*/genome_annotation_training/...` — the sibling project `Fungi_BFD`, not `Fungi_BFD_runs` — i.e. a stale absolute path baked in before the runs tree was reorganized/copied into `Fungi_BFD_runs`. Script relinked 4,122 species from missing/broken `training` to the canonical absolute path, gated on a completeness check (`funannotate_train.pasa.gff3` non-empty, `transcript.alignments.bam` non-empty, no `.pasa_train_failed` marker — mirrors FUNANNOTATE_TRAIN's own success markers in `nextflow/modules/funannotate/utils.nf`/`predict/FUNANNOTATE_TRAIN/main.nf`). Post-apply: 5,808 valid symlinks, 0 broken symlinks. 6,916 species genuinely have no canonical training dir (never trained, expected/normal), 76 have an empty/failed `pasa.gff3`, 44 have an explicit `.pasa_train_failed` marker — none of these were touched (no valid source to link to).

**776-species duplicate-directory reconciliation — resolved 2026-09-10**: 776 species had a *real* (non-symlink) `training/` directory directly under `genome_annotation/<SPECIES>/`, and every one of these also already had a `genome_annotation_training/<SPECIES>/training` counterpart — i.e. duplicated training data, not missing data. `scripts/one-off/reconcile_training_duplicates.py` written to reconcile per-species (policy confirmed with Jason: canonical wins when it is complete, has no broken internal symlinks, and is the same age or newer than the `genome_annotation/` copy by `pasa.gff3` mtime; anything else reported as ambiguous rather than auto-resolved). Outcome:
- **752** resolved automatically (canonical wins) — `genome_annotation/` copy deleted, replaced with a symlink to canonical.
- **9** had an aborted PASA run in the `genome_annotation/` copy (working `pasa/` dir present, no finished `funannotate_train.pasa.gff3`) while canonical had a real finished one — manually verified canonical was complete (non-empty `pasa.gff3` + `transcript.alignments.bam`, no `.pasa_train_failed` marker) then deleted+relinked the same way: `Agaricus_bisporus_A10_9`, `Agaricus_bisporus_A33_62`, `Agaricus_bisporus_ARP23`, `Agaricus_bisporus_H39`, `Agaricus_sinodeliciosus_ZRL20160001-S65`, `Candida_tropicalis_MYCT-2728-2021`, `Candida_tropicalis_U873`, `Saccharomyces_cerevisiae_NCYC2592`, `Saccharomyces_cerevisiae_wine017`.
- **15** had NO finished `funannotate_train.pasa.gff3` on *either* side — genuinely aborted training, not a reconciliation case at all (both stop at the `pasa/` working dir + `trinity.alignments.bam`). 11/15 are `Saccharomyces_paradoxus` engineered strains (`::kanMX4`/`::natMX4/hphNT1` markers) sharing what may be a common interrupted batch, though no log evidence was checked to confirm a shared cause. Both copies deleted for all 15 (`Alternaria_alternata_P02PL2`, `Aspergillus_fumigatus_SPS-2`, `Dichotomocladium_elegans_RSA_919-`, `Saccharomyces_mikatae_SDPGP3=15`, and the 11 *S. paradoxus* strains) so `FUNANNOTATE_TRAIN` reattempts them fresh. Confirmed via `nextflow/modules/funannotate/predict/FUNANNOTATE_TRAIN/main.nf`'s own skip logic (`TRAIN_GFF3`/`.pasa_train_failed` check) that these were *already* going to be reattempted regardless — neither marker existed before deletion — so the deletion is disk cleanup of stale partial PASA state, not what triggers the retrain.

**Final verified state (2026-09-10, `genome_annotation/*/training` census)**: 6,569 valid symlinks, 0 broken symlinks, 0 real (duplicate) directories remaining. Combined with the earlier missing/broken-symlink repair (4,122 relinked), all three failure modes found in this incident (dangling symlink, missing entry, duplicated real directory) are now fully resolved across the dataset. Both one-off scripts moved to `scripts/one-off/` (not meant for regular/repeated use — they're incident-specific repairs, not a pipeline component). The actual pre-`predict` preflight *check* (a standing guard against this recurring) is still not implemented — see [[training-symlink-preflight-check]] / `todo/training_symlink_preflight_check.md`.
