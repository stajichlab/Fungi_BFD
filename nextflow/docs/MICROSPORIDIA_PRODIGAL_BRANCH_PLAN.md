...
```bash
    ...
    # (see Task 5 Step 6 below): augustus/snap/glimmerhmm's training/reuse
    # still happens (fast, via a pre-existing --busco_seed_species entry's
    # info.json, not a fresh from-scratch BUSCO run against this genome's
    # own too-few complete models -- Microsporidia_predict/STATUS.md:73-82's
    # 36-vs-200 failure), but their EVM weight of 0 excludes their output
    # from the final gene models regardless of what that training produced.
