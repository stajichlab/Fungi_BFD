...
- **Every ad-hoc/manual Nextflow test run (smoke tests, `-stub-run`, real small-scale validation runs, standalone probe scripts) must use an isolated directory under `.nf_launch/<UNIQUE_NAME>/`** — never launch a test from the shared production `launchDir`/`workDir` (e.g. a plain `nextflow run` from `nextflow/` reuses `nextflow/work`, and `nextflow.config`'s directory-scoped auto-loading means params like `params.target`/`params.training_target` silently default to the real production paths unless overridden). A real pipeline run may already have jobs in flight against those same directories/paths; reusing them for a test risks colliding with or corrupting live state, not just wasting compute. Pass `-w .nf_launch/<UNIQUE_NAME>/work` and pass real output-path params (`--target`, `--out_dir`, etc.) as `.nf_launch/<UNIQUE_NAME>/output/...` explicitly on the command line — a bare in-script `params.x = value` (whether at top-of-script or inside `workflow { }`) does NOT reliably override a config-set default; confirmed empirically 2026-09-10 that a `workflow{}`-body reassignment of `params.target` was silently ignored, while the identical value passed as `--target` on the CLI worked. Pick `<UNIQUE_NAME>` specific to the test (date + short description), never reused across unrelated test setups.
